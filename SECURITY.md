...
`SecureBytes`는 자신이 소유한 전용 버퍼를 마지막 참조 해제 직전에 `memset_s`로 덮어씁니다.
이 보장은 생성에 사용한 원본 `String`이나 `Data`, Security.framework 또는 호출자가 만든
다른 복사본에는 적용되지 않습니다. 기존 `Data` API도 복사와 최적화가 가능하므로 패키지가
모든 메모리 복사본을 zeroize한다고 보장하지 않습니다. 호출자는 Secret 수명을 짧게 유지하고
불필요한 문자열 변환과 장기 캐시를 피해야 합니다.
